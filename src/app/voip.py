...
def testRegister():
    ...
    result, reason = yield user.bind('"Bala Murugan" <sip:b2m@a-cti.com>', username='bala', password='password')
...
def testCall():
    ...
    user1.address = Address('"Bala" <sip:bala@bala.com>')
    user1.username, user1.password = 'bala', 'password'
    result, reason = yield user1.bind(user1.address) 
    
    user2 = User(sock2).start()
    user2.address = Address('"murugan" <sip:murugan@bala.com>')
    user2.username, user2.password = 'murugan', 'bala'
    
    multitask.add(testIncoming(user1))
    yield multitask.sleep(2)

    yield testOutgoing(user2, 'sip:500@50.112.112.39')
